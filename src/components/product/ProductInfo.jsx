    ...
    <div className="lg:col-span-5 flex flex-col justify-start space-y-6 px-4 md:px-0 pb-10">
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center">
          ...
          <button className="text-[10px] font-bold border-b border-kicks-dark uppercase cursor-pointer">
            Size Chart
          </button>
        </div>
        <div className="grid grid-cols-4 md:grid-cols-5 gap-2">
          {(product?.sizes || [38, 39, 40, 41, 42]).map((size) => (
            <button
              key={size}
      ...
      <div className="space-y-3 pt-2">
        <div className="flex gap-2">
          ...
          >
            <Heart size={20} className={product?.isFavorite ? "fill-white" : ""} />
          </Button>
        </div>

        <Button 
          variant="primary" 
          className="w-full py-4 text-xs tracking-widest shadow-none"
        >
          Buy It Now
        </Button>
      </div>
